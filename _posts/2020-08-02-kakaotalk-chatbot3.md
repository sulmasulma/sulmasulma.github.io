...
Amazon `S3`라는 Data Lake에 데이터를 저장한다. 절차를 다시 정리하면 아래와 같다.
...
각 트랙들의 수치를 사용하는 것이 아니고 수치의 **평균** 을 사용하는 것이기 때문에 정보를 많이 잃어버리게 되지만, 아직 프로토타입으로 관련 아티스트를 서빙하는 것이므로 일단 간단하게 계산했다.
...
아래와 같이 검색한 아티스트와 관련 아티스트를 가로로 넘기면서 볼 수 있다.
...
2편을 작성한 이후로 2달이 넘게 걸렸다. 내용이 적은 것은 아니었지만 나의 게으름이 한 몫 했다. 프로토타입을 우선 개발하기 위해 Euclidean Distance를 계산하는 단순한 방법을 사용했지만, 이후에는 모델링을 적용해 보려고 한다. 이를 위해선 수천 개 이상의 아티스트 데이터가 필요하다. 그만큼의 Spotify 아티스트 ID가 필요한데, 어떻게 수집해야 할지 고민 중이다. 아예 다른 데이터를 사용하는 것도 고려 중이다. 추천 알고리즘에 대한 공부도 더 필요하다.
