...
- 번외: [챗봇 업데이트](https://sulmasulma.github.io/data/2023/01/15/kakaotalk-chatbot-update.html)
...
- *수정: 현재 UI가 살짝 변경되었다. 기본값이 코드 인라인 편집이고, `.zip` 파일을 사용하거나 S3에서 파일을 참조할 경우 우측에 있는 작업 버튼을 누르면 된다.*
  ![20200603-17-lambdaui](/assets/20200603-17-lambdaui.png)
...
`성시경`을 사용자 발화로 입력하고, `@sys.person.name`엔티티를 이용한 `solo`라는 파라미터로 설정했다고 가정하자. 상단 메뉴에 있는 **봇테스트** 창을 열어 `성시경`을 입력하면, Lambda 코드에서 입력한 대로 응답이 정상적으로 수신되는 것을 확인할 수 있다.
